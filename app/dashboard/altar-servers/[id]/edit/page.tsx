...
export default function EditAltarServerPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const router = useRouter()
  const [altarServer, setAltarServer] = useState<AltarServer | null>(null)
  const [formData, setFormData] = useState({
    ...
    joinedDate: "",
    status: "active" as AltarServer["status"],
    notes: "",
  })

  useEffect(() => {
    const data = dataStore.getAltarServerById(id)
    if (data) {
      setAltarServer(data)
      setFormData({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    dataStore.updateAltarServer(id, {
  ...
  if (!altarServer) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-amber-700">Monaguillo no encontrado</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-amber-900">Editar Monaguillo</h1>
          <p className="text-amber-700 mt-1">
            {altarServer.firstName} {altarServer.lastName}
          </p>
        </div>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.status} onValueChange={(v) => setFormData({ ...formData, status: v as AltarServer["status"] })}>
                  <SelectTrigger className="border-amber-200">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>

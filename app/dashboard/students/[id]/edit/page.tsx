...
import { getStudent, StudentPayload, updateStudent, uploadStudentPhoto } from "@/lib/students"
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (studentData.parents && Array.isArray(studentData.parents)) {
        const existingParentIds = studentData.parents.map((p: any) => p.id)
      ...
      setFormData({
        ...
        birth_date: studentData.birth_date ? new Date(studentData.birth_date).toISOString().split("T")[0] : "",
        grade: studentData.grade || "",
        baptism_date: studentData.sacrament?.baptism_date
          ? new Date(studentData.sacrament.baptism_date).toISOString().split("T")[0]
          : "",
        first_communion_date: studentData.sacrament?.first_communion_date
          ? new Date(studentData.sacrament.first_communion_date).toISOString().split("T")[0]
          : "",
        confirmation_date: studentData.sacrament?.confirmation_date
          ? new Date(studentData.sacrament.confirmation_date).toISOString().split("T")[0]
          : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updates = {
      ...
      sacrament: {
        ...
        first_communion_date: formData.first_communion_date ? formData.first_communion_date : undefined,
        confirmation_date: formData.confirmation_date ? formData.confirmation_date : undefined,
      },
      allergies: formData.allergies || undefined,
      medical_conditions: formData.medical_conditions || undefined,
      status: formData.status,
      photo_url: formData.photo_url || undefined,
      parents: selectedParentIds.join(","),
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        if (response.photo_url) {
          // ¡Y MIRA ESTE LOG!
          console.log("Paso 3: Guardando URL en el estado:", response.photo_url);
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-amber-900 mb-2">Editar Estudiante</h1>
        <p className="text-amber-700">Actualiza la información del estudiante</p>
      </div>
